...
void PlanFootprints::init_setup(const Affine3d init_leg_pose[], const Quat& waist_r, const Vector3& com, const double endcpoff[]) {
  for (int i = 0; i < 2; ++i) {
  ...
  land_pos << 0.0, 0.0, 0.0;
}


void PlanFootprints::setValues(walking_state ws, rl swing_leg, Vector3 landpos) {
  wstate = ws;
  swingleg = swing_leg;
  land_pos = landpos;
}
